...
The temperature and humidity ranges are configurable via defining min and max values. The frequency of readings is configurable;. I have found a reading every 15 minutes to provide adequate granularity while not creating a deluge of data.

The application is implemented entirely in Python and follows a micro-service architecture approach. There is a client application collecting the temperature/humidity readings and sending them to a server application via a RESTful API. 

The client side code is running on a Raspberry Pi 4 connected to an Adafruit Si7021 temperature/humidity sensor. Adafruit provides a Python library for the sensor, [adafruit-circuitpython-si7021 3.2.1](https://pypi.org/project/adafruit-circuitpython-si7021/), which makes reading data from the sensor very straight-forward. The [Requests](https://requests.readthedocs.io/en/master/) library is used to send the data to the server via an HTTP POST.
...
I implemented a suite of tests which send 'canned' readings to the server via the RESTful API. It in essence works like the client, though rather than taking readings from a sensor, it reads data from a CSV file. This makes testing the logic of the server much easier than heating/cooling/etc the physical sensor in order to produce out of range readings. These tests are run via an app which can be run either on the server or on the client via [Pytest](https://docs.pytest.org/en/stable/).


# Setup
This application leverages two web-based services. The first, MongoDB Atlas, provides a managed MongoDB instance. MongoDB is a (perhaps 'the') leading NoSQL document store database. The second web-based service is SendGrid, which is an email service. Both offer a free tier of services which is more than sufficient for this application. And they both provide easy to use and well documented Python interface libraries.

#### MongoDB Atlas
If you do not already have a MongoDB Atlas  account, you can set one up at [https://www.mongodb.com/cloud/atlas](https://www.mongodb.com/cloud/atlas). Note the free account is more than sufficient for running this application.

As part of the account creation process, you will set up a project and a MongoDB cluster. Once these are set up, it is a good idea to set up some additional security measures, including a non-administrative user ID for accessing the database and a restriction on the IP addresses that can connect to the database:
1. In the MongoDB Atlas dashboard, select the cluster you set up
2. On the left, you will see a menu with an option under heading Security, titled Database Access; choose this item.
...
5. In the menu on the left, select Network Access:
	...
	2. Add the externally visible IP address of the computer you will be using as the server. To find the externally visible address, open a browser and search for "my IP Address". 

Now, we will get the connection string that will be used to connect to the MongoDB cluster:
1. Clusters under heading Data Storage in the menu on the left. 
2. Click on Connect.
3. In the dialog that appears, click on Connect your application.
4. For Driver, select Python and Version 3.6 or later.
5. Copy the connection string; it should look like the following.
```mongodb+srv://razpi:<password>@cluster0-ylplp.mongodb.net/<dbname>?retryWrites=true&w=majority```
...
4. Click on Create & View; make note of this key as it will be needed later on

To authenticate the sender email address, click on Settings on the menu on the left and:
1. Click on Sender Authentication.
2. Click on Verify a Single Sender.
3. Enter the requested information and click on Create.
...
One last thing to set up is the IP address of the server in the test code. In file api_server\test_main.py, change the following line (is at about line 33) to the IP address of the PC running the server code (be careful to NOT change the port number):
...
And as we did above for the test code, we need to set the IP address of server PC. In file razpi_client\razpi_client.py, change the following line (is at about line 12) to the IP address of the PC running the server code (be careful to NOT change the port number):
