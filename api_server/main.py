...
def send_alert_notification_email(dev_id, alert_type, current_value):
    ...
    Args:
        ...
        alert_type (str):       Type of alert; either 'temp' or 'humidity'
...
def is_an_existing_active_alert(db, dev_id, alert_type):
    ...
    Args:
        ...
        alert_type (str):     Type of alert to check for; 'temp' or 'humidity'

    Returns:
        True if there is an existing alert of type alert_type for dev_id; else False
    """
    query = {'dev_id': dev_id,
             'type': alert_type}
...
def renotify_if_notification_delay_exceeded(db, dev_id, alert_type, current_value):
    """
    Called when we continue in an alert condition for the specified device / alert type
    ...
    """
    if TRACE_MESSAGE_PROCESSING:
        print('  ==> renotify_if_notification_delay_exceeded({}, {}, {})'.format(dev_id, alert_type, current_value))
    ...
    if elapsed_time_minutes >= CONFIG_DATA['alert_renotification_delay']:
        ...
        send_alert_notification_email(dev_id, alert_type, current_value)


def handle_alert_condition(db, dev_id, alert_type, current_value):
    """
    Handles action(s) to take for an alert condition for the specified device / alert type.
    ...
    """
    if TRACE_MESSAGE_PROCESSING:
        print('  ==> handle_alert_condition({}, {}, {})'.format(dev_id, alert_type, current_value))
    if is_an_existing_active_alert(db, dev_id, alert_type):
        renotify_if_notification_delay_exceeded(db, dev_id, alert_type, current_value)
...
def clear_active_alert_if_is_one(db, dev_id, alert_type, current_value):
    """
    Handles the transition from being in an alert condition to no longer being in an alert condition.
    ...
    """
    if TRACE_MESSAGE_PROCESSING:
        print('  ==> clear_active_alert_if_is_one({}, {}, {})'.format(dev_id, alert_type, current_value))
    if is_an_existing_active_alert(db, dev_id, alert_type):
        ...
        send_alert_cleared_notification_email(dev_id, alert_type, current_value)


def continue_current_condition(db, dev_id, alert_type, current_value):
    """
    Handles continuation of the current condition (i.e. continuation of an active alert or
    continuation of no alert).
    ...
    """
    if TRACE_MESSAGE_PROCESSING:
        print('  ==> continue_current_condition({}, {}, {})'.format(dev_id, alert_type, current_value))
...
def get_active_alerts_counts(dev_id: str = Query(None,
                             ...
                             alert_type: str = Query(None,
                                                     alias='alert-type',
                                                     description='Alert type; \'temp\' or \'humidity\'')):
    # Note the docstring is picked up by the OpenAPI doc tools, thus only include info
    # that makes sense from an API end-user's perspective.
    """
    ...
    to return the count for. If not specified, a total count of active alerts is returned. alert-type
    is the alert type (temp or humidity) to return the count for. If not specified, a count of
    both is returned.
    """
    if TRACE_MESSAGE_PROCESSING:
...
def get_alert_history_counts(dev_id: str = Query(None,
    ...
    The request supports two optional parameters. dev-id, is the ID for the device
    to return the count for. If not specified, a total count of alert history is returned. alert-type
    is the alert type (temp or humidity) to return the count for. If not specified, a count of
    both is returned.
    """
    if TRACE_MESSAGE_PROCESSING:
